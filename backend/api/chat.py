...
from service.cache import get_history , append_message
...
async def chat_text(payload: chat):
    ...
    reply_text = generate_reply(payload.user_input, crisis=crisis_flag, is_first=payload.is_first , history=history)
...
@router.post("/voice")
async def chat_voice(file: UploadFile = File(...), is_first: bool = False, session_id: str | None = Form(None)):
    """
    Handles voice input:
